...
#testing variables:
test_sents = [
              "The procedure for managing emergency landing of the aircraft was not effective",
              "The NOTAMS were delivered with delate",
              "There was not appropriate risk analysis for assistance under dangerous conditions",
              "Absence of operative procedure to describe displacement positions"
              ]
shell_tag = "S"
embedding_size = 128
...
#cleaned_sents e sents_vect have same index
for index, test_vect in enumerate(sents_vect):
    ...
    for i, hf in enumerate(factors):
        key_hfs[i] = hf
        sents = hf.get_sents()
        similarity[hf.get_name()] = 0 #initialized
    ...
    if found_hf == None:
        print("Target sentence: ", cleaned_sents[index])
